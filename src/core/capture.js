 ...
 * @param {string[]} [options.includeFontFamilies=[]]
...
export async function captureDOM(element, options = {}) {
  ...
  ({ clone, classCSS } = await prepareClone(
    element,
    compress,
    embedFonts,
    options
  ...
  if (embedFonts) {
    await new Promise((resolve) => {
      idle(
        async () => {
          fontsCSS = await embedCustomFonts({ includeFontFamilies });
          resolve();
        },
        { fast }
      );
      ...
      () => {
        ...
        if (!hasScale && (hasW || hasH)) {
          ...
          clone.style.transform =
            `${scaleTransform} ${existingTransform}`.trim();
